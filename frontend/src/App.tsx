...
function AutomationTimer({ action, now, detailed = false }: {
  ...
  if (action.status === "INCIDENT_HELD") return <div className="automation-timer retry" title="Correlated provider failures engaged the circuit breaker"><ShieldCheck size={13}/><div><strong>Held by failure swarm</strong><span>Canary resumes after a five-minute quiet window</span></div></div>;
...
function statusTone(status: string) {
  ...
  if (["PARTIALLY_RECOVERED", "WAITING", "RETRYING", "RETRY_SCHEDULED", "INCIDENT_HELD", "PROPOSED"].includes(status)) return "warning";
  return "neutral";
}
...
function CaseDrawer({ detail, providerMode, whatsappMode, publicBaseUrl, busy, onClose, onAction }: {
  detail: CaseDetail;
  ...
  const smartRecoveryUrl = detail.recoverySession ? `${publicBaseUrl}/recover/${encodeURIComponent(detail.recoverySession.id)}` : null;
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="case-drawer">
      ...
      <section className="drawer-section"><div className="section-row"><div><span className="innovation-label">PayArc innovation</span><h3>Recovery Decision Passport</h3></div>{activeAction && <StatusPill status={activeAction.status} />}</div>
        {activeAction ? <><div className="decision-passport"><div><TrendingUp/><span>Causal proof</span><strong>{causalProof}</strong></div><div><Route/><span>Path conservation</span><strong>{pathDecision}</strong></div><div><ShieldCheck/><span>Safety envelope</span><strong>{safetyEnvelope}</strong></div></div><div className="decision-card"><div className="decision-title"><Bot size={18} /><strong>{humanize(activeAction.type)}</strong><span>{humanize(activeAction.decision.provider)} · {Math.round(activeAction.decision.confidence * 100)}%</span></div><p>{activeAction.decision.reason}</p><div className="policy-line"><ShieldCheck size={15} /><span>{activeAction.policy.allowed ? "Deterministic policy checks passed" : activeAction.policy.reasons.join("; ")}</span></div><AutomationTimer action={activeAction} now={now} detailed/>{activeAction.error && activeAction.status !== "RETRY_SCHEDULED" && <div className="action-error"><AlertTriangle size={15}/><span>{activeAction.error}</span></div>}{activeAction.status === "RETRY_SCHEDULED" && activeAction.error && <div className="retry-note"><AlertTriangle size={14}/><span>Last attempt: {activeAction.error}. PayArc will retry automatically.</span></div>}{smartRecoveryUrl && <div className="smart-session"><div><Radio size={15}/><span><strong>Permanent Smart Recovery Link</strong><small>{humanize(detail.recoverySession?.status)} · {detail.recoverySession?.openCount ?? 0} customer opens</small></span></div><div><a href={smartRecoveryUrl} target="_blank" rel="noreferrer">Open <ExternalLink size={13}/></a><button className="secondary-button" onClick={() => void navigator.clipboard.writeText(smartRecoveryUrl)}><Copy size={13}/>Copy</button></div></div>}{activeAction.providerUrl && <div className="provider-link"><a href={activeAction.providerUrl} target="_blank" rel="noreferrer">Direct Razorpay fallback <ExternalLink size={13} /></a></div>}</div></> : <EmptyState icon={<Bot />} title="No intervention" detail="This case has no active action." />}
        {activeAction?.decision.counterfactual && <div className="counterfactual-card"><TrendingUp size={16}/><div><span>Counterfactual estimate</span><strong>{formatMoney(activeAction.decision.counterfactual.expectedIncrementalValue, item.currency ?? "INR")} expected incremental value</strong><small>{Math.round(activeAction.decision.counterfactual.interventionRecoveryProbability * 100)}% with intervention vs {Math.round(activeAction.decision.counterfactual.naturalRecoveryProbability * 100)}% natural · rejected {humanize(activeAction.decision.counterfactual.rejectedAction)}</small></div></div>}
...
function Overview({ data, onView, onOpenCase }: { data: Snapshot; onView: (view: ViewId) => void; onOpenCase: (id: string) => void }) {
  ...
  return <>
    ...
    <section className="panel automation-strip"><div><Radio/><span>Smart sessions</span><strong>{data.metrics.smartRecoverySessions} live · {data.metrics.smartSessionOpens} opens</strong></div><ChevronRight/><div><Bot/><span>Decisions</span><strong>{data.config.autoActionsEnabled ? "Automatic" : "Approval gated"}</strong></div><ChevronRight/><div><Zap/><span>Swarm shield</span><strong>{data.metrics.retriesPrevented} retries held</strong></div><ChevronRight/><div><Send/><span>WhatsApp</span><strong>{data.config.whatsappMode === "cloud_api" && data.config.whatsappAutoSendEnabled ? "Auto-send + reply intent" : data.config.whatsappAutoSendEnabled ? "Auto-prepare" : "Manual fallback"}</strong></div><button onClick={() => onView("integrations")}>View controls</button></section>
...
function ScenariosView({ scenarios, history, testLab, runningId, realTestBusy, onRun, onInspect, onLaunchReal, onOpenCheckout, onOpenCase, onView }: {
  scenarios: DemoScenario[];
  ...
  const workspaceDemos = scenarios.filter((item) => item.mode === "workspace" && item.destinationView);
  ...
  return <>
    ...
    {workspaceDemos.length > 0 && <section className="panel revenue-demo-launcher"><div className="panel-heading"><div><span className="overline">One-click merchant demo</span><h3>Demo every Revenue Autopilot page</h3><p>Each run creates fresh, high-priority data in the selected workspace and gives you a direct link to continue the workflow.</p></div><span className="count-badge">{workspaceDemos.length} flows</span></div><div className="revenue-demo-grid">{workspaceDemos.map((scenario) => {
      const icon = scenario.destinationView === "overview" ? <LayoutDashboard/> : scenario.destinationView === "portfolio" ? <TrendingUp/> : scenario.destinationView === "incidents" ? <Network/> : scenario.destinationView === "journeys" ? <Route/> : scenario.destinationView === "subscriptions" ? <RefreshCw/> : scenario.destinationView === "receivables" ? <Building2/> : <MessageSquareText/>;
      return <article key={scenario.id}><div className={`revenue-demo-icon accent-${scenario.accent}`}>{icon}</div><div><span>{scenario.destinationLabel}</span><strong>{scenario.title}</strong><p>{scenario.description}</p></div><div className="revenue-demo-actions"><button className="demo-view-link" onClick={() => onView(scenario.destinationView!)}>View page</button><button className="demo-run-button" disabled={Boolean(runningId)} onClick={() => onRun(scenario)}>{runningId === scenario.id ? <><RefreshCw className="spin"/>Running…</> : <><Play/>Create demo</>}</button></div></article>;
    })}</div></section>}
    <div className="lab-banner panel"><div className="lab-icon"><TestTube2 /></div><div><span className="overline">Complete proof suite</span><h3>Workspace demos plus isolated failure and security simulations</h3><p>Revenue Autopilot demos create visible merchant records. Provider edge cases, replay attacks and forged signatures remain isolated so they never pollute real recovery reporting.</p></div><div className="lab-stats"><strong>{scenarios.length}</strong><span>repeatable flows</span></div></div>
    ...
    <section className="panel scenario-history"><div className="panel-heading"><div><span className="overline">Scenario evidence ledger</span><h3>Scenario run history</h3><p>Workspace demos record what they created; security and provider simulations retain an isolated trace.</p></div><span className="count-badge">{history.length}</span></div>{history.length ? <div className="scenario-history-list">{history.map((run) => <button onClick={() => onInspect(run.runId)} key={run.runId}><i className={`accent-${run.accent}`}/><div className="history-main"><strong>{run.title}</strong><span>{run.observed}</span></div><div><span>Outcome</span><StatusPill status={run.outcome}/></div><div><span>Evidence</span><strong>{run.eventCount} events · {run.auditProofCount} proofs</strong></div><time>{formatDate(run.ranAt)}</time><ChevronRight size={17}/></button>)}</div> : <EmptyState icon={<FileClock/>} title="No scenario runs yet" detail="Run any demo above; its outcome and proof will appear here."/>}</section>
  </>;
}
...
function ScenarioTraceDrawer({ run, onClose }: { run: ScenarioResult; onClose: () => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="case-drawer scenario-trace-drawer">
      <div className="drawer-head"><div><span className="overline">{run.scenario.mode === "workspace" ? "Merchant workspace proof" : "Isolated sandbox trace"}</span><h2>{run.scenario.title}</h2></div><button className="icon-button" onClick={onClose}><X size={19}/></button></div>
      ...
      <section className="drawer-section"><div className="section-row"><div><span className="innovation-label">{run.scenario.mode === "workspace" ? "Workspace mutation" : "Ingress boundary"}</span><h3>{run.scenario.mode === "workspace" ? "Created revenue state" : "Signed event trace"}</h3></div><span className="sandbox-chip">{run.scenario.mode === "workspace" ? "Demo workspace" : "Sandbox only"}</span></div>{run.eventTrace.length ? <div className="trace-events">{run.eventTrace.map((event, index) => <div key={event.id}><i>{index + 1}</i><div><strong>{event.type}</strong><span>{humanize(event.entityType)} · {compactId(event.entityId)}</span></div><StatusPill status={event.status}/><code>{event.payloadHash.slice(0, 10)}…</code></div>)}</div> : <div className="trace-empty"><ShieldCheck/><div><strong>{run.scenario.mode === "workspace" ? "Fresh workspace state created" : "No event was persisted"}</strong><span>{run.scenario.mode === "workspace" ? "This demo writes directly to the Revenue Autopilot model; open its destination page to continue the workflow." : "The forged signature was rejected before ingestion, which is the expected security outcome."}</span></div></div>}</section>

      <section className="drawer-section"><div className="section-row"><div><span className="innovation-label">Decision boundary</span><h3>Policy and actions</h3></div></div>{run.case && <div className="trace-case"><div><span>Sandbox case</span><strong>{compactId(run.case.id, 12)}</strong></div><div><span>Failure class</span><strong>{humanize(run.case.failureClass)}</strong></div><div><span>At risk</span><strong>{formatMoney(run.case.amount, run.case.currency ?? "INR")}</strong></div><div><span>Recovered</span><strong className="green-text">{formatMoney(run.case.recoveredAmount, run.case.currency ?? "INR")}</strong></div></div>}{run.actions.length ? <div className="trace-actions">{run.actions.map((action) => <article key={action.id}><div><Bot size={17}/><strong>{humanize(action.type)}</strong><StatusPill status={action.status}/></div><p>{action.decision.reason}</p><footer><span>{Math.round(action.decision.confidence * 100)}% confidence</span><span>{action.policy.allowed ? "Policy allowed" : action.policy.reasons.join("; ")}</span></footer></article>)}</div> : <div className="trace-empty"><ShieldCheck/><div><strong>{run.scenario.mode === "workspace" ? "Continue on the destination page" : "No action created"}</strong><span>{run.scenario.mode === "workspace" ? "The page-specific buttons advance this demo through its visible, bounded workflow." : "The scenario stopped safely before an intervention was authorized."}</span></div></div>}</section>
...
function IntegrationsView({ config }: { config: PublicConfig }) {
  ...
  const webhookUrl = `${config.publicBaseUrl}/webhooks/razorpay`;
  const copy = async () => { await navigator.clipboard.writeText(webhookUrl); setCopied(true); window.setTimeout(() => setCopied(false), 1500); };
  const aiName = config.aiProvider === "groq" ? "Groq free-model decisions" : config.aiProvider === "openai" ? "OpenAI structured decisions" : "Deterministic engine";
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {scenarioResult && <div className="result-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setScenarioResult(null)}><div className="result-modal panel"><button className="icon-button result-close" onClick={() => setScenarioResult(null)}><X/></button><div className="result-success"><CheckCircle2/></div><span className="overline">{scenarioResult.scenario.mode === "workspace" ? "Merchant demo ready" : "Scenario completed in isolated sandbox"}</span><h2>{scenarioResult.scenario.title}</h2><p>{scenarioResult.observed}</p><div className="result-comparison"><div><span>Expected</span><strong>{scenarioResult.scenario.expected}</strong></div><ChevronRight/><div><span>Observed state</span><strong>{humanize(scenarioResult.outcome)}</strong></div></div><div className="scenario-proof-grid"><div><span>{scenarioResult.scenario.mode === "workspace" ? "Workspace objects" : "Persisted events"}</span><strong>{scenarioResult.scenario.mode === "workspace" ? String(scenarioResult.security.workspaceObjectsCreated ?? 0) : scenarioResult.eventTrace.length}</strong></div><div><span>Worker runs</span><strong>{scenarioResult.workerRuns.length}</strong></div><div><span>Actions</span><strong>{scenarioResult.actions.length}</strong></div><div><span>Audit proofs</span><strong>{scenarioResult.recentAudit.length}</strong></div></div>{Object.keys(scenarioResult.security).length > 0 && <div className="security-proof">{Object.entries(scenarioResult.security).filter(([key]) => key !== "workspaceObjectsCreated").map(([key, value]) => <div key={key}><span>{humanize(key)}</span><strong>{String(value)}</strong></div>)}</div>}<div className="result-actions">{scenarioResult.scenario.destinationView && <button className="result-destination" onClick={() => { const destination = scenarioResult.scenario.destinationView!; setScenarioResult(null); switchView(destination); }}><ArrowUpRight/>Open {scenarioResult.scenario.destinationLabel ?? "result"}</button>}<button onClick={() => { setScenarioTrace(scenarioResult); setScenarioResult(null); }}><ListFilter/>Inspect proof</button><button className="secondary-button" onClick={() => { setScenarioResult(null); switchView("scenarios"); }}><FlaskConical/>Run another</button></div></div></div>}
