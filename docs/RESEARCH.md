...
- The preferred hackathon adapter uses Groq's free-plan `openai/gpt-oss-20b`, which supports strict JSON Schema output. Groq currently publishes free limits of 30 requests/minute and 1,000/day for this model. Malformed, unavailable, or rate-limited responses fall back deterministically. [Groq structured outputs](https://console.groq.com/docs/structured-outputs) · [Groq rate limits](https://console.groq.com/docs/rate-limits)
- The optional OpenAI adapter remains available through the Responses API with strict JSON Schema output, no tools, minimized inputs, and `store: false`. [Official OpenAI Responses API](https://developers.openai.com/api/reference/cli/resources/responses/methods/create)
